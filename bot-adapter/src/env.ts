...
function env(keys: string | string[], cb?: EnvParseCallback): any {
	...
	if (keys.length > 1) {
		for (let key of keys) {
			...
			if (isDefined(value)) return cb ? cb(value, envName) : value;
		}
...
export const MODE: Mode = env(["MODE", "NODE_ENV"], (v) => {
	...
	if (!predicate.includes(v.toLowerCase() as any))
		throw new TypeError("Invalid application mode");
